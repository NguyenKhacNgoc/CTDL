...
void NhapHD (HoaDon &hd)
{
	...
	fflush(stdin);
	cout<<"Nguoi lap: ";gets(hd.NguoiLap);
	fflush(stdin);
	cout<<"Ngay lap: ";gets(hd.NgayLap);
	fflush(stdin);
...
void Sua_HD (ListHD &HD)
{
	...
	for (p = HD.Head;p!=NULL;p=p->next)
	{
		...
		{
			fflush(stdin);
			cout<<"\nNguoi lap moi: ";gets(NguoiLapMoi);
			strcpy(p->info.NguoiLap, NguoiLapMoi);
		}
	}
	cout<<"\n----------Sua thanh cong----------\n";
...
void Dem_HD (ListHD HD)
{
	...
	for (p=HD.Head;p!=NULL;p=p->next)
	{
		if (p->info.tongtien > tongtiencandem)
		{
			dem++;
		}
	}
...
void MenuHD()
{
	...
	do
	{
		...
		cout<<"\n 1. Nhap danh sach hoa don va luu file tu dong";
		...
		switch(menu)
		{
			case 1:NhapDSHD(HD);Luufile_HD(f, HD);break;
			case 2:InDSHD(HD);break;
			case 3:Luufile_HD(f, HD);break;
			case 4:Docfile_HD(f, HD);break;
			case 5:Edit_HD(HD);Luufile_HD(f, HD);break;
